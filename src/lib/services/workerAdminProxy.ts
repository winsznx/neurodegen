...
function workerMisconfigured(): NextResponse {
  return NextResponse.json(
    { error: 'WORKER_ADMIN_URL is not set', code: 'WORKER_ADMIN_URL_MISSING' },
    { status: 503 },
  );
}
...
function workerUnreachable(detail: string): NextResponse {
  return NextResponse.json(
    { error: 'Worker did not respond', code: 'WORKER_UNREACHABLE', detail },
    { status: 502 },
  );
}
...
export async function proxyAdminRequest(
  ...
  try {
    ...
    return workerUnreachable(err instanceof Error ? err.message : String(err));
  } finally {
...
export async function fetchWorkerStatusRaw(): Promise<
  { ok: true; status: unknown } | { ok: false; code: string; detail: string }
> {
  const base = getWorkerAdminUrl();
  ...
  try {
    ...
    if (!response.ok) return { ok: false, code: 'WORKER_HTTP_ERROR', detail: `status ${response.status}` };
    const json = (await response.json()) as { agent?: unknown };
    if (!json.agent) return { ok: false, code: 'WORKER_PAYLOAD_INVALID', detail: 'no agent field' };
